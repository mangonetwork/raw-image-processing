...
def hdf5_file_info(sitefile, datadir, latlondir):
    ...
    for site in site_list:
        ...
        print(code, site_name, site_lon, site_lat)
            # read lat/lon from where ever Latitude.csv and Longitude.csv are for that site
        try:
            latitude = np.array(pd.read_csv(os.path.join(latlondir,'{}/calibration/Latitudes.csv'.format(site_name)), dtype=float, delimiter=','))
            longitude = np.array(pd.read_csv(os.path.join(latlondir,'{}/calibration/Longitudes.csv'.format(site_name)), dtype=float, delimiter=','))
        ...
        #I need to start working here onwards.
        print(os.walk(sitepath))
        print(sitepath)
        #print(os.walk(sitepath)[2])
        for procFolder in next(os.walk(sitepath))[2]:
            print(procFolder)
            date = dt.datetime.strptime(procFolder[1:3]+'/'+procFolder[3:5]+ '/20'+ procFolder[5:7], '%b/%d/%Y')
